<template>
    <div class="flex flex-col min-h-screen bg-gray-100">
        <!-- Header -->
        <header class="bg-[#001E62] sticky top-0 z-50 shadow-md">
            <div class="max-w-[1200px] mx-auto px-4">
                <div class="flex justify-between items-center h-14">
                    <!-- Logo -->
                    <div class="flex-shrink-0">
                        <a href="/" class="text-2xl font-bold text-white">
                            <img src="/src/theme-aquiesportes2/assets/logo.webp" width="147" height="32" alt="Logo" title="Logo" loading="eager" fetchpriority="high">
                        </a>
                    </div>
                        ...
                        <template v-for="category in mainNavCategories.rootCategories" :key="category.id">
                            <div v-if="mainNavCategories.childrenMap[category.id]" class="relative">
                                <button
                                    ...
                                    class="dropdown-toggle text-white hover:bg-[#001545] dark:hover:bg-[#001E62] hover:text-[#ffcc00] px-2 py-1 rounded text-sm flex items-center whitespace-nowrap transition-colors"
                                    :class="{'bg-[#001545] text-[#ffcc00]': openDropdowns[category.id]}"
                                >
                                    {{ category.name }}
                                ...
                                <div
                                    ...
                                    class="dropdown-menu absolute left-0 mt-1 w-48 rounded-md shadow-lg bg-[#001E62] ring-1 ring-black ring-opacity-5 z-10"
                                >
                                    <a v-for="child in mainNavCategories.childrenMap[category.id]" :key="child.id"
                                        :href="`/category/${child.slug}`"
                                        class="block text-white hover:bg-[#001545] hover:text-[#ffcc00] px-3 py-2 text-sm transition-colors"
                                    >
                                        {{ child.name }}
                                    </a>
                                </div>
                            ...
                            <a v-else
                                ...
                                class="text-white hover:bg-[#001545] dark:hover:bg-[#001E62] hover:text-[#ffcc00] px-2 py-1 rounded text-sm whitespace-nowrap transition-colors"
                            >
                                {{ category.name }}
                            </a>
                        </template>
                ...
                <!-- Mobile Menu -->
                <div v-show="mobileMenuOpen" class="md:hidden py-3 border-t border-[#001545]">
                    <div class="flex flex-col gap-1">
                        <template v-for="category in mainNavCategories.rootCategories" :key="category.id">
                            <div v-if="mainNavCategories.childrenMap[category.id]" class="w-full">
                                <button
                                    @click="(e) => toggleDropdown(category.id, e)"
                                    class="dropdown-toggle flex items-center justify-between w-full text-white hover:bg-[#001545] dark:hover:bg-[#001E62] hover:text-[#ffcc00] rounded px-3 py-2 text-sm"
                                    :class="{'bg-[#001545] text-[#ffcc00]': openDropdowns[category.id]}"
                                >
                                    {{ category.name }}
                                    <svg class="ml-1 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div v-show="openDropdowns[category.id]" class="pl-4 py-1 bg-[#001545] rounded mt-1">
                                    <a
                                        v-for="child in mainNavCategories.childrenMap[category.id]"
                                        :key="child.id"
                                        :href="`/category/${child.slug}`"
                                        class="block px-4 py-2 text-sm text-white hover:bg-[#001E62] hover:text-[#ffcc00] rounded"
                                    >
                                        {{ child.name }}
                                    </a>
                                </div>
                            ...
                            <a
                                v-else
                                :href="`/category/${category.slug}`"
                                class="block text-white hover:bg-[#001545] dark:hover:bg-[#001E62] hover:text-[#ffcc00] rounded px-3 py-2 text-sm"
                            >
                                {{ category.name }}
                            </a>
                        </template>
                    ...
                    <div class="flex justify-center space-x-6 mt-3 pt-3 border-t border-[#001545]">
            ...
            <div class="max-w-[1200px] mx-auto px-4 py-8">
                ...
                <div class="border-t border-gray-700 mt-8 pt-6 text-center">
                    <p class="text-gray-400 text-sm">
                        &copy; {{ new Date().getFullYear() }} Aqui Esportes - Todos os direitos reservados.
                    </p>
                </div>
            </div>
        </footer>
...
const scripts = computed(() => {
    ...
    // Optimize Google Analytics scripts for better performance
    const optimizedGAScripts = settingsStore.googleAnalyticsScripts.map((script: any) => ({
        ...script,
        async: true,
        defer: script.defer !== false
    }));
    
    return [...baseScripts, ...optimizedGAScripts];
});
...
useHead({
    ...
    link: [
        ...
        {
            rel: 'preload',
            href: '/src/theme-aquiesportes2/style.css',
            as: 'style'
        },
        {
            rel: 'preload',
            href: '/src/theme-aquiesportes2/assets/logo.webp',
            as: 'image',
            fetchpriority: 'high'
        },
        {
        ...
        },
        {
            rel: 'icon',
            type: 'image/ico',
            href: '/src/theme-aquiesportes2/assets/favicon.ico'
        },
        // External connections with preconnect for better performance
        ...
        { rel: 'preconnect', href: 'https://www.googletag.com/', crossorigin: true },
        { rel: 'preconnect', href: 'https://connect.facebook.net/', crossorigin: true },
        ...
        { rel: 'preconnect', href: 'https://static.aquiesportes.com.br/', crossorigin: true },
        // DNS prefetch for faster lookups
        { rel: 'dns-prefetch', href: 'https://www.googletagmanager.com/' },
        { rel: 'dns-prefetch', href: 'https://www.google-analytics.com/' },
        { rel: 'dns-prefetch', href: 'https://securepubads.g.doubleclick.net' },
        { rel: 'dns-prefetch', href: 'https://static.aquiesportes.com.br/' },
        { rel: 'dns-prefetch', href: 'https://platform.twitter.com' },
        { rel: 'dns-prefetch', href: 'https://embed.reddit.com' },
