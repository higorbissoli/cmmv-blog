<template>
    <div class="w-full relative">
        <div class="lg:max-w-4xl md:max-w-3xl px-4 sm:px-6 mx-auto">
            <div class="bg-white rounded-lg px-4 sm:px-6 py-8">
                <h1 class="text-3xl font-bold mb-6 text-neutral-900">Sobre nós</h1>
                <a id="about" title="Sobre nós"></a>
                <p class="mb-6 text-neutral-700 text-justify">Conheça mais sobre o <strong>Aqui Esportes</strong>, nossa missão e a equipe que trabalha diariamente para trazer as melhores notícias e conteúdos sobre o universo esportivo.</p>

                <h2 class="text-2xl font-semibold mt-8 mb-4 text-neutral-800">Nossa História</h2>
                <a id="our-history" title="Nossa História"></a>
                <p class="mb-4 text-neutral-700 text-justify">O <strong>Aqui Esportes</strong> nasceu da paixão por esportes e da necessidade de criar um portal que oferecesse conteúdo de qualidade, atualizado e acessível para todos os amantes do esporte brasileiro e internacional.</p>
                <p class="mb-6 text-neutral-700 text-justify">Desde nossa fundação, temos o compromisso de manter nossos leitores informados sobre as principais novidades do mundo esportivo, sempre com credibilidade, imparcialidade e paixão pelo que fazemos.</p>

                <h2 class="text-2xl font-semibold mt-8 mb-4 text-neutral-800">Nossa Missão</h2>
                <a id="mission" title="Nossa Missão"></a>
                <p class="mb-6 text-neutral-700 text-justify">Nossa missão é ser a principal fonte de informação esportiva no Brasil, oferecendo conteúdo de qualidade, análises aprofundadas e cobertura completa dos principais eventos esportivos nacionais e internacionais, sempre com transparência e respeito aos nossos leitores.</p>

                <h2 class="text-2xl font-semibold mt-8 mb-4 text-neutral-800">Nossa Visão</h2>
                <a id="vision" title="Nossa Visão"></a>
                <p class="mb-6 text-neutral-700 text-justify">Ser reconhecido como um portal de esportes confiável e abrangente, contribuindo para o desenvolvimento da cultura esportiva brasileira e conectando fãs de esportes de todas as modalidades.</p>

                <h2 class="text-2xl font-semibold mt-8 mb-4 text-neutral-800">Nossos Valores</h2>
                <a id="values" title="Nossos Valores"></a>
                <ul class="mb-6 pl-5 list-disc text-neutral-700 text-justify">
                    <li class="mb-3"><strong>Credibilidade:</strong> Comprometemo-nos em verificar todas as informações antes da publicação, mantendo sempre a veracidade dos fatos.</li>
                    <li class="mb-3"><strong>Imparcialidade:</strong> Apresentamos os fatos de forma objetiva, sem favorecimento a clubes, atletas ou modalidades específicas.</li>
                    <li class="mb-3"><strong>Paixão pelo Esporte:</strong> Nossa equipe é formada por verdadeiros apaixonados por esporte, que vivem e respiram a cultura esportiva.</li>
                    <li class="mb-3"><strong>Inovação:</strong> Estamos sempre buscando novas formas de apresentar o conteúdo e melhorar a experiência dos nossos usuários.</li>
                    <li class="mb-3"><strong>Transparência:</strong> Mantemos sempre a clareza em nossas informações e processos editoriais.</li>
                </ul>

                <h2 class="text-2xl font-semibold mt-8 mb-4 text-neutral-800">O que Cobrimos</h2>
                <a id="coverage" title="O que Cobrimos"></a>
                <p class="mb-4 text-neutral-700 text-justify">No <strong>Aqui Esportes</strong>, você encontra cobertura completa de:</p>
                <ul class="mb-6 pl-5 list-disc text-neutral-700 text-justify">
                    <li class="mb-2"><strong>Futebol:</strong> Campeonatos nacionais e internacionais, Copa do Mundo, seleções e mercado da bola</li>
                    <li class="mb-2"><strong>Basquete:</strong> NBA, NBB, seleção brasileira e competições internacionais</li>
                    <li class="mb-2"><strong>Vôlei:</strong> Superliga, seleções brasileiras masculina e feminina, Liga das Nações</li>
                    <li class="mb-2"><strong>Tênis:</strong> Grand Slams, ATP, WTA e tenistas brasileiros</li>
                    <li class="mb-2"><strong>Fórmula 1:</strong> Corridas, bastidores e pilotos brasileiros</li>
                    <li class="mb-2"><strong>Esportes Olímpicos:</strong> Atletismo, natação, ginástica e muito mais</li>
                    <li class="mb-2"><strong>E-Sports:</strong> Competições de jogos eletrônicos e cenário competitivo</li>
                </ul>

                <h2 class="text-2xl font-semibold mt-8 mb-4 text-neutral-800">Nossa Equipe</h2>
                <a id="team" title="Nossa Equipe"></a>
                <p class="mb-4 text-neutral-700 text-justify">Nossa equipe é composta por profissionais apaixonados por esporte, que trabalham incansavelmente para trazer:</p>
                <ul class="mb-6 pl-5 list-disc text-neutral-700 text-justify">
                    <li class="mb-2">Notícias em tempo real</li>
                    <li class="mb-2">Análises técnicas e táticas</li>
                    <li class="mb-2">Conteúdo multimídia de qualidade</li>
                </ul>

                <h2 class="text-2xl font-semibold mt-8 mb-4 text-neutral-800">Compromisso com a Qualidade</h2>
                <a id="quality-commitment" title="Compromisso com a Qualidade"></a>
                <p class="mb-4 text-neutral-700 text-justify">Estamos comprometidos em oferecer:</p>
                <ul class="mb-6 pl-5 list-disc text-neutral-700 text-justify">
                    <li class="mb-2"><strong>Conteúdo Verificado:</strong> Todas as informações passam por rigorosa checagem antes da publicação</li>
                    <li class="mb-2"><strong>Atualizações Constantes:</strong> Nosso site é atualizado 24 horas por dia, 7 dias por semana</li>
                    <li class="mb-2"><strong>Diversidade de Fontes:</strong> Utilizamos múltiplas fontes confiáveis para garantir a precisão das informações</li>
                    <li class="mb-2"><strong>Responsabilidade Editorial:</strong> Seguimos rigorosos padrões jornalísticos e éticos</li>
                </ul>

                <h2 class="text-2xl font-semibold mt-8 mb-4 text-neutral-800">Parcerias e Colaborações</h2>
                <a id="partnerships" title="Parcerias e Colaborações"></a>
                <p class="mb-4 text-neutral-700 text-justify">O <strong>Aqui Esportes</strong> mantém parcerias estratégicas com:</p>
                <ul class="mb-6 pl-5 list-disc text-neutral-700 text-justify">
                    <li class="mb-2">Confederações e federações esportivas</li>
                    <li class="mb-2">Clubes e organizações esportivas</li>
                    <li class="mb-2">Agências de notícias especializadas</li>
                    <li class="mb-2">Fotógrafos e correspondentes esportivos</li>
                </ul>

                <div class="bg-blue-50 border-l-4 border-[#001E62] p-6 mb-6">
                    <div class="flex">
                        <div class="flex-shrink-0">
                            <svg class="h-6 w-6 text-[#001E62]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                            </svg>
                        </div>
                        <div class="ml-3">
                            <h3 class="text-lg font-medium text-neutral-800 mb-2">Nossa Paixão</h3>
                            <p class="text-sm text-neutral-700">
                                Acreditamos que o esporte tem o poder de unir pessoas, inspirar mudanças e criar momentos inesquecíveis. 
                                É por isso que trabalhamos com tanto amor e dedicação para levar até você as melhores histórias do mundo esportivo.
                            </p>
                        </div>
                    </div>
                </div>

                <h2 class="text-2xl font-semibold mt-8 mb-4 text-neutral-800">Contato</h2>
                <a id="contact-info" title="Contato"></a>
                <p class="mb-4 text-neutral-700 text-justify">Quer saber mais sobre nós ou tem alguma sugestão? Entre em contato:</p>
                <p class="mb-4 text-neutral-700"><strong>E-mail:</strong> <code class="px-1.5 py-0.5 bg-gray-100 rounded">higor@tos.world</code></p>
                <p class="mb-6 text-neutral-700">Siga-nos também em nossas redes sociais para acompanhar nosso dia a dia e interagir com nossa equipe.</p>

                <div class="text-center mt-8 p-6 bg-gradient-to-r from-[#001E62] to-[#0056b3] rounded-lg text-white">
                    <h3 class="text-xl font-bold mb-2">Obrigado por fazer parte da nossa comunidade!</h3>
                    <p class="text-gray-200">
                        Juntos, continuamos escrevendo a história do esporte brasileiro e mundial.
                    </p>
                </div>
            </div>
        </div>
    </div>
</template>

<script setup lang="ts">
import { useHead } from '@unhead/vue';

useHead({
    title: 'Sobre nós - Aqui Esportes',
    meta: [
        {
            name: 'description',
            content: 'Conheça o Aqui Esportes, nossa missão, valores e equipe dedicada a trazer as melhores notícias esportivas.'
        },
        {
            name: 'keywords',
            content: 'Aqui Esportes, sobre nós, missão, história, equipe, valores, esportes'
        }
    ]
})
</script> 